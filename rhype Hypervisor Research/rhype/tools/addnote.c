...
main(int ac, char **av)
{
     ...
     printf("phoff=%d, phnum=%d, phentsize=%d, nnote=%d, n=%d\n",
	    ph, np, ps, nnote, n);
     ...
     {
	  printf("operation does not fit into buffer\n");
	  goto nospace;
     }
     ...
     for (i = 0; i < ps + nnote; ++i)
	  if (buf[ph + i] != 0)
	  {
	       printf("ph=%d, nnote=%d, ps=%d\n", ph, nnote, ps);
	       printf("i=%d\n",i);
	       printf("space not zero filled as expected\n");
	       goto nospace;
	  }
